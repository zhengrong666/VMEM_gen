...
sys.path.append('/nfs/home/zhangdongrong/thinclient_drives/VMEM_gen/src')

from mem import MemChunk, MemFile
from prince import prince, sbox  # type: ignore
from secded_gen import ecc_encode_some  # type: ignore
from secded_gen import load_secded_config


ROM_BASE_WORD = 0x3a000000 // 8
ROM_SIZE_WORDS = 4096
...
class Scrambler:
    ...
    def flatten(self, mem: MemFile) -> MemFile:
        ...
        print([hex(i) for i in flattened.chunks[0].words])        
    ...
    def add_hash(self, scr_mem: MemFile) -> None:
        ...
        assert scr_mem.width == 64

        scr_chunk = scr_mem.chunks[0]

        bytes_per_word = 64 // 8
        num_digest_words = 256 // 64
        ...
        for log_addr in range(self.rom_size_words - num_digest_words):
            ...
            to_hash += scr_word.to_bytes(bytes_per_word, byteorder='little')
            print(hex(scr_word))
            print(scr_word.to_bytes(bytes_per_word, byteorder='little'))
        ...
        digest256_ori = int.from_bytes(digest_bytes, byteorder='little')
        print(hex(digest256_ori))

        digest256_bin = '1001111010010010110011001010111100111000001111001110101010011001001110010101010101100011101011101011100111001000101001101100000000111100011011011010001000100000000010011000001010100111101100111110110011001011000111110101101010011001011001011011100001001'
        digest256 = int(digest256_bin,2)
        print(hex(digest256))
        # Chop the 256-bit digest into 64-bit words. These words should never
        ...
        for digest_idx in range(num_digest_words):
            ...
            w32 = (digest256 >> (64 * digest_idx)) & mask64
            # found_mismatch = False

            # for chk_bits in range(128):
            #     w39 = w32 | (chk_bits << 64)
            #     clr39 = self.unscramble_word(64, log_addr, w39)
            #     clr32 = clr39 & mask64
            ...
            print(hex(w32))
            with open("./real_4096", 'w') as file:
                for word in scr_chunk.words:
                    file.write(f"{word:016x}\n")
...
def main() -> int:
    ...
    clr_mem = MemFile.load_elf64(args.infile, 8 * ROM_BASE_WORD)
